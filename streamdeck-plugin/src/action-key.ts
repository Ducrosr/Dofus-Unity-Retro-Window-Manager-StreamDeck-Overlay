...
export type ActionKind = "launch" | "next" | "previous" | "move-up" | "move-down" | "refresh" | "toggle-ignore";
...
const labels: Record<"fr" | "en" | "es", Record<ActionKind, string>> = {
	fr: { launch: "OUVRIR", next: "SUIVANT", previous: "PRÉC.", "move-up": "MONTER", "move-down": "BAISSER", refresh: "SCAN", "toggle-ignore": "IGNORER" },
	en: { launch: "OPEN", next: "NEXT", previous: "PREV.", "move-up": "MOVE UP", "move-down": "MOVE DOWN", refresh: "SCAN", "toggle-ignore": "IGNORE" },
	es: { launch: "ABRIR", next: "SIG.", previous: "ANT.", "move-up": "SUBIR", "move-down": "BAJAR", refresh: "ESCANEAR", "toggle-ignore": "IGNORAR" },
};
...
export abstract class ThemeAwareAction extends SingletonAction<EmptySettings> {
	...
	private async render(action: KeyAction, state: BridgeState): Promise<void> {
		...
		const glyph = glyphs[this.actionKind];
		const label = connected ? labels[language][this.actionKind] : "DWM OFF";
		const glyphSize = glyph.length > 2 ? 30 : 66;
		const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="144" height="144" viewBox="0 0 144 144"><rect width="144" height="144" rx="18" fill="${palette.bg}"/><rect x="5" y="5" width="134" height="134" rx="15" fill="none" stroke="${connected ? palette.accent : palette.line}" stroke-width="4"/><path d="M18 106h108" stroke="${palette.line}" stroke-width="2" opacity=".65"/><text x="72" y="62" fill="${connected ? palette.fg : palette.muted}" font-family="Arial, sans-serif" font-size="${glyphSize}" font-weight="800" text-anchor="middle" dominant-baseline="middle">${glyph}</text><text x="72" y="122" fill="${connected ? palette.accent : palette.muted}" font-family="Arial, sans-serif" font-size="14" font-weight="800" text-anchor="middle">${label}</text></svg>`;
