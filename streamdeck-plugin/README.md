...
La version 0.6.1 signale en orange les fenêtres qui demandent l’attention, avec un léger clignotement facultatif synchronisé avec l’application. Elle peut afficher le portrait ainsi qu’une des 39 icônes officielles de caractéristiques ou 20 icônes officielles de métiers intégrées, et reprend automatiquement le thème ainsi que la langue transmis par l’application. Les douze thèmes de l’application sont compatibles avec Unity comme avec Retro. Elle conserve le secours Windows des actions Personnage, Suivant et Précédent : si Stream Deck garde le premier plan, le plugin réduit sa propre fenêtre puis répète la commande une fois. Le script PowerShell fourni vérifie strictement que la fenêtre active appartient bien à Stream Deck avant toute réduction.
