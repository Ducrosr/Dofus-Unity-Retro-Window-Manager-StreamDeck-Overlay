...
def _set_popup_idle(window) -> None:
    """Keep the popup HWND alive while rendering the existing OBS-keyable idle surface."""
    try:
...
def _style_obs_popup(window) -> None:
    _set_window_title(window, POPUP_WINDOW_TITLE)
    _clear_toolwindow_style(window)


def enable_obs_overlay_capture() -> None:
    """Enable stable production overlay HWNDs without global Toplevel substitution.

    The idle popup surface remains unchanged on purpose: validating a different
    inactive surface belongs to the real OBS test campaign, not this hardening pass.
    """
    ...
    if not getattr(ensure_original, "_dwm_obs_capture_wrapper", False):
        ...
        def ensure_obs_persistent(self) -> None:
            ...
            if getattr(self, "window_role", "production") != "production":
                return
    ...
    if not getattr(init_original, "_dwm_obs_capture_wrapper", False):
        ...
        def init_obs_overlay(self, *args, **kwargs) -> None:
            ...
            if getattr(self, "window_role", "production") == "production":
                self.enable_obs_capture(
                    popup_idle=_set_popup_idle,
                    popup_style=_style_obs_popup,
                    popup_title=POPUP_WINDOW_TITLE,
                )
