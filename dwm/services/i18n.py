...
_EN: dict[str, str] = {
    ...
    "DWM crée automatiquement une Capture de fenêtre persistante par client détecté dans cette scène, sans limite fixe de 8 clients. Une nouvelle capture n’est créée que lorsqu’un client supplémentaire apparaît ; pendant les rotations, DWM change uniquement la visibilité des captures déjà initialisées.": "DWM automatically creates one persistent Window Capture per detected client in this scene, with no fixed eight-client limit. A new capture is created only when an additional client appears; during rotations, DWM only changes the visibility of already initialized captures.",
...
_ES: dict[str, str] = {
    ...
    "DWM crée automatiquement une Capture de fenêtre persistante par client détecté dans cette scène, sans limite fixe de 8 clients. Une nouvelle capture n’est créée que lorsqu’un client supplémentaire apparaît ; pendant les rotations, DWM change uniquement la visibilité des captures déjà initialisées.": "DWM crea automáticamente una Captura de ventana persistente por cada cliente detectado en esta escena, sin un límite fijo de ocho clientes. Solo se crea una nueva captura cuando aparece un cliente adicional; durante las rotaciones, DWM solo cambia la visibilidad de las capturas ya inicializadas.",
